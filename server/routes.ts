  ...
  wss.on("connection", (ws: WebSocket) => {
    ...
    ws.on("message", async (data: Buffer) => {
      try {
        ...
        switch (message.type) {
          case "create_room": {
            const { username, roomCode, server } = message;
            console.log(`Creating room - Username: ${username}, Server: ${server}`);
            ...
            const player = await storage.createPlayer({
              ...
              markedNumbers: [],
              isHost: true,
              server: server || 'origins',
            });
            console.log(`Created player with server: ${player.server}`);

            const playersInRoom = await storage.getPlayersByRoomId(room.id);
            console.log(`Players in room:`, playersInRoom.map(p => ({ username: p.username, server: p.server })));
          ...
          case "join_room": {
            const { username, roomCode, server } = message;
            console.log(`Joining room - Username: ${username}, Server: ${server}`);
          ...
          case "mark_number": {
            ...
            if (!markedNumbers.includes(number) && number !== "FREE") {
              ...
              if (hasWon) {
                // Parar el juego inmediatamente y establecer verificación pendiente
                ...
                if (player.isHost) {
                  ...
                  // Si un jugador no-host gana, establecer verificación pendiente y bloquear el juego
                  await storage.updateRoom(room.id, { 
                    isGameActive: false,
                  ...
                  // Notificar a TODOS que alguien cantó bingo y el juego está bloqueado
                  players.forEach((p) => {
                    wss.clients.forEach((client) => {
                      if (
                        client.readyState === WebSocket.OPEN &&
                        ...
                        if (p.isHost) {
                          ...
                          // A todos los demás les enviamos que alguien cantó bingo y no pueden cerrar el modal
                          client.send(
                            JSON.stringify({ 
          ...
          case "confirm_bingo": {
            ...
            // Parar el juego y limpiar el estado de verificación pendiente
            ...
            // Generar nuevos cartones para todos los jugadores y anunciar el ganador
            const players = await storage.getPlayersByRoomId(room.id);
          ...
          case "reject_bingo": {
            ...
            if (!host || !host.isHost) {
              ws.send(
                JSON.stringify({ type: "error", message: "Solo el host puede rechazar el bingo" })
              );
              return;
            }

            const room = await storage.getRoomById(host.roomId!);
            if (!room || !room.pendingBingoVerification) {
              ws.send(
                JSON.stringify({ type: "error", message: "No hay bingo pendiente de verificación" })
              );
              return;
            }

            // Limpiar el estado de verificación pendiente y reanudar el juego
            await storage.updateRoom(room.id, { 
              isGameActive: true,
              pendingBingoVerification: null
            });

            // Reanudar el intervalo del juego
            const interval = setInterval(() => {
              callNextNumber(room.id, wss);
            }, 6000);
            gameIntervals.set(room.id, interval);

            // Notificar a todos que el bingo fue rechazado y el juego continúa
            const players = await storage.getPlayersByRoomId(room.id);
            players.forEach((p) => {
              wss.clients.forEach((client) => {
                if (
                  ...
                  client.send(
                    JSON.stringify({ 
                      type: "bingo_rejected",
                      message: "El host ha rechazado el bingo. El juego continúa."
                    })
                  );
                }
              });
